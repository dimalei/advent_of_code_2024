...
class Maze:
        ...
        while len(queue) > 0 and self.exit_node == None:
            ...
            for edge in current_node.edges:
                ...
                if new_node in queue:
                    ...
                    elif new_node.cost == queue[index].cost:
                        queue[index].trail |= new_node.trail
                    continue
                # if node was already processed, check for similarity and add trail if same
                elif new_node in self.processed_nodes:
                    index = self.processed_nodes.index(new_node)
                    if new_node.cost == self.processed_nodes[index].cost:
                        self.processed_nodes[index].trail |= new_node.trail
                    continue
    ...
    def print_cheapest(self):
        ...
        # adding +2 for the start and exit
        print(f"total trails: {len(self.exit_node.trail) + 2}")
